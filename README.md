...
graph TD
    ...
    subgraph "Nube (Ej. AWS / Azure / GCP)"
        B[Load Balancer]
        
        subgraph "Contenedores Docker (EKS / ECS)"
            C[Servidor Frontend]
            D[Servidor Backend API]
            E[Worker de Análisis (Scanner)]
        end
        
        subgraph "Servicios Administrados"
            F[(Base de Datos RDS)]
            G[(Almacenamiento S3 / Blobs)]
        end
    end
...
## 6. Arquitectura de Software (Vista General)

```mermaid
architecture-beta
    group api(cloud)[API Layer]
    group core(server)[AnzenCore Services]
    group db(database)[Data Storage]

    service web(internet)[Web App]
    service gateway(server)[API Gateway] in api
    service scanner(cpu)[Scanner Engine] in core
    service auth(key)[Auth Service] in core
    service sql(database)[Postgres DB] in db
    service files(disk)[APK Storage] in db
